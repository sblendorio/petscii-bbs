...
public class WordpressProxy extends PetsciiThread {
    ...
    protected void listPosts() throws Exception {
        ...
        for (Map.Entry<Integer, Post> entry: posts.entrySet()) {
            ...
            String line = WordUtils.wrap(filterPrintable(HtmlUtils.htmlClean(post.title)), 36, "\r", true);
            println(line.replaceAll("\r", "\r  " + (i>9 ? SPACE : EMPTY)));
        }
        newline();
    }
    ...
    protected void logo() throws Exception {
        write(logo);
        write(GREY3);
    }
