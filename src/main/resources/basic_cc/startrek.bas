...
6810 INPUT "Enter initial coordinates (X,Y) ";XY$: IF XY$="." THEN GOSUB 20000: GOTO 6810
6815 GOSUB 10000: SRCX=X9:SRCY=Y9
6820 INPUT "Enter final coordinates (X,Y) ";XY$: IF XY$="." THEN GOSUB 20000: GOTO 6820
6825 GOSUB 10000: DSTX=X9:DSTY=Y9
...
8210 RETURN
10000 W0 = INSTR(XY$, ","): IF W0=0 THEN X9=VAL(XY$):Y9=0 : RETURN
10010 X9=VAL(MID$(XY$,1,W0)): Y9=VAL(MID$(XY$,W0+1))
10020 RETURN
20000 INPUT "QUIT: ARE YOU SURE";KK$
20010 IF KK$<>"Y" AND KK$<>"S" AND KK$<>"." THEN RETURN
20020 END
