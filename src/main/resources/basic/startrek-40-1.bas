...
440 Q9=X+8*Y:GOSUB 1500:MEM(INT(Q)+INT(Q9))=INT(C9)+40:PRINT "! ";STR$(K2);STR$(B2);STR$(S2);" ";
...
550 J5=INT(K5/5):K5=K5-J5:E1=E1-INT(J5/R):PRINT INT(J5/R);" UNITS OF PHASER DAMAGE":GOSUB 1800:RETURN
...
820 FOR X=0 TO 7:Q9=X+8*Y:GOSUB 1500:P=MEM(INT(Q)+INT(Q9))
...
1000 B=S:GOSUB 1100:Q9=Q0:GOSUB 1500:MEM(INT(Q)+INT(Q9))=INT(C9)+40
1010 N=S2:V=1:L=2:GOSUB 1200
1020 N=K2:V=3:L=4:GOSUB 1200:K5=100:Y5=INT(I/8):X5=I-8*Y5
1030 N=B2:V=2:L=3:GOSUB 1200:Y6=INT(I/8):X6=I-8*Y6
1035 C$="GREEN":IF K2<>0 THEN C$="RED!"
1036 PRINT:PRINT "STARDATE: ";STR$(3278-T);",  CONDITION: ";C$
1040 PRINT "QUADRANT ";STR$(X1);",";STR$(Y1);"  -  SECTOR ";STR$(X2);",";STR$(Y2)
1050 P=MEM(INT(S)+INT(S0)):MEM(INT(S)+INT(S0))=4:IF P=0 THEN RETURN
...
1085 GOTO 990
1100 FOR K=0 TO 63:MEM(INT(B)+INT(K))=0:NEXT K:RETURN
1200 IF N=0 THEN RETURN
1205 FOR K=1 TO N
1210 I=INT(RND(1)*64):P=MEM(INT(B)+INT(I)):IF P+V>=L THEN 1210
1215 MEM(INT(B)+INT(I))=INT(P)+INT(V):NEXT K:RETURN
1300 E1=500:E2=3:RETURN
1400 Y0=INT(E0/64):X0=E0-64*Y0:Y1=INT(Y0/8):X1=INT(X0/8):Y2=Y0-8*Y1:X2=X0-8*X1
1410 Q0=X1+8*Y1:S0=X2+8*Y2:R=ABS (X5-X2)+ABS (Y5-Y2):RETURN
1500 C9=MEM(INT(Q)+INT(Q9)):IF C9>=40 THEN C9=C9-40
1510 S2=C9-10*INT(C9/10):K2=INT(C9/20):B2=INT((C9-S2-20*K2)/10):RETURN
1600 FOR J=1 TO N:PRINT C$;:NEXT J:PRINT MID$(C$,1,1):RETURN
1800 IF E1>0 THEN RETURN
1810 C$="ENERGY":GOTO 980
1900 PRINT "*** BOOM ***":K1=K1-1:K2=0:MEM(INT(Q)+INT(Q0))=MEM(INT(Q)+INT(Q0))-20:MEM(INT(S)+INT(X5)+8*INT(Y5))=0
...
2020 P=3*MEM(INT(S)+INT(X)+8*INT(Y))+1:IF X=0 THEN PRINT "!";
