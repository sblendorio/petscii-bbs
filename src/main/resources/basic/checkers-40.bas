...
1590 U1=0:V1=0: PRINT "FROM";:GOSUB 2000:E=X9:H=Y9:X=E:Y=H: IF S((8*Y+X)-8)<=0 THEN 1590
1670 PRINT "TO";:GOSUB 2000:A=X9:B=Y9:X=A:Y=B: IF S((8*Y+X)-8)<>0 OR ABS (A-E)>2 OR ABS (A-E)<> ABS (B-H) THEN 1670
1750 S((8*B+A)-8)=S((8*H+E)-8):S((8*H+E)-8)=0: IF ABS (E-A)<>2 THEN 1810
1800 X1=(E+A)/2:Y1=(H+B)/2:S((8*Y1+X1)-8)=0
1802 PRINT "+TO";:GOSUB 2000: X1=X9:Y1=Y9: IF X1<1 THEN 1810
...
1999 END
2000 INPUT XY$:IF XY$="" THEN 2000
2010 IF XY$="." THEN GOSUB 2100
2020 CP=INSTR(XY$,",")
...
2070 GOTO 2000
2100 INPUT "QUIT: ARE YOU SURE";KK$
2110 IF KK$<>"Y" AND KK$<>"S" AND KK$<>"." THEN RETURN
2120 END
