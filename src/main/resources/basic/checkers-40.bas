...
2 PRINT TAB(16);"CHECKERS" : PRINT "CREATIVE COMPUTING MORRISTOWN NEW JERSEY "
3 PRINT "COMPUTER IS X, AND YOU ARE O.      (1,1) IS THE LOWER LEFT CORNER"
4 PRINT "(1,8) IS THE UPPER LEFT CORNER     (8,1) IS THE LOWER RIGHT CORNER"
5 PRINT "(8,8) IS THE UPPER RIGHT CORNER    '+TO' = ANOTHER? TYPE 0 FOR NO"
...
1230 PRINT "FROM ";R(2);",";R(3);" TO ";R(4);",";R(5);:R(1)=-99
...
1590 U1=0:V1=0: INPUT "FROM",XY$:GOSUB 2000:E=X9:H=Y9:X=E:Y=H: IF S((8*Y+X)-8)<=0 THEN 1590
1670 INPUT "TO",XY$:GOSUB 2000:A=X9:B=Y9:X=A:Y=B: IF S((8*Y+X)-8)<>0 OR ABS (A-E)>2 OR ABS (A-E)<> ABS (B-H) THEN 1670
1750 S((8*B+A)-8)=S((8*H+E)-8):S((8*H+E)-8)=0: IF ABS (E-A)<>2 THEN 1810
1800 X1=(E+A)/2:Y1=(H+B)/2:S((8*Y1+X1)-8)=0
1802 INPUT "+TO",XY$:GOSUB 2000: X1=X9:Y1=Y9: IF X1<1 THEN 1810
...
1999 END
2000 IF XY$="" THEN X9=0:Y9=0:RETURN
2010 CP=INSTR(XY$,",")
2020 IF CP=0 THEN X9=VAL(XY$) : GOTO 2040
2030 IF CP=1 THEN X9 = 0 ELSE X=VAL(MID$(XY$,1,CP-1))
2040 IF CP=0 THEN Y9 = 0 ELSE Y=VAL(MID$(XY$,CP+1))
2050 RETURN
