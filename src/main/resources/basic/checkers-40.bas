    ...
    2 TAB (16): PRINT "CHECKERS" : PRINT "CREATIVE COMPUTING MORRISTOWN NEW JERSEY "
 ...
 1400 RETURN
 1420 PRINT : PRINT : FOR Y=8 TO 1 STEP -1: FOR X=1 TO 8: TAB(X*4): IF S((8*Y+X)-8)=0 THEN PRINT ".";
 ...
 1590 U1=0:V1=0: INPUT "FROM",E,H:X=E:Y=H: IF S((8*Y+X)-8)<=0 THEN 1590
 1670 INPUT "TO",A,B:X=A:Y=B: IF S((8*Y+X)-8)<>0 OR ABS (A-E)>2 OR ABS (A-E)<> ABS (B-H) THEN 1670
 1750 S((8*B+A)-8)=S((8*H+E)-8):S((8*H+E)-8)=0: IF ABS (E-A)<>2 THEN 1810
 1800 X1=(E+A)/2:Y1=(H+B)/2:S((8*Y1+X1)-8)=0
 1802 INPUT "+TO",X1,Y1: IF X1<1 THEN 1810
